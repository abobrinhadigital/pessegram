...
module Pessegram
  class Bot
    def initialize(token, master_id, gemini_key, persona_path, goiabook_config = {}, gemini_model = 'gemini-2.0-flash',
      ...
      persona_interpolated = format(persona_raw, master_id: master_id).force_encoding('UTF-8')
      @gemini = GeminiClient.new(gemini_key, persona_interpolated, gemini_model)
      @memory = Memory.new(10) # 5 do usuário + 5 do modelo
      @goiabook = if goiabook_config[:url] && goiabook_config[:token]
                    GoiabookClient.new(goiabook_config[:url],
                                       goiabook_config[:token])
                  end
      @mangofier = if mangofier_config[:url] && mangofier_config[:token]
                     MangofierClient.new(mangofier_config[:url],
                                         mangofier_config[:token])
                   end
    end
    ...
    def handle_message(bot, message)
      ...
      return unless @access_control.authorized?(user_id)
      ...
      when '/start'
        ...
        bot.api.send_message(chat_id: message.chat.id,
                             text: 'Saudações, meu mestre. O Pessegram está ativo. Memória limpa. O que falhou hoje?')
      when '/mapear'
      ...
      when %r{^/mapear}
        bot.api.send_message(chat_id: message.chat.id,
                             text: 'Mestre, para usar o mapeamento ativo, digite: `/mapear [LINK_DO_MANGAUPDATES]`')
      when %r{https?://\S+}
        ...
        is_mangofier_hint = message.reply_to_message&.from&.is_bot && original_text.match?(%r{mangaupdates\.com/series/[a-z0-9]+}i)
    ...
    def save_link(bot, message, url, silent: false)
      unless @goiabook
        return bot.api.send_message(chat_id: message.chat.id,
                                    text: 'GoiabookLM não configurado, mestre.')
      end
    ...
    def forward_to_mangofier(bot, message, url)
      unless @mangofier
        return bot.api.send_message(chat_id: message.chat.id,
                                    text: 'MangofierService não configurado, mestre.')
      end
